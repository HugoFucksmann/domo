...
import { cacheImages } from "../helper/carcheImages";
import {
  dataJuegoCerebro,
...
class AppProvider extends Component {
  ...
  handlerStart = () => {
    ...
    if (perfil.includes("f")) {
      let f = dataJuegoCerebro.filter(
        (obj) =>
          obj.num === 19 || obj.num === 20 || obj.num === 24 || obj.num === 38
      );
  ...
  handlerPersonaOp = (e, option, edad) => {
    if (e && option.op === "b" && edad < 16) {
      ...
      let filter = this.state.datosPersona.tipo.filter((ops) => ops !== option);
      this.setState({
  ...
  handleMsjFinalCerebro = () => {
    function handleSelect(datosPersona) {
      ...
      if (datosPersona.tipo.length === 1) {
        ...
        datosPersona.tipo.filter((obj) => primero.includes(obj.op)).length > 0
      ) {
        return "h";
      } else if (
        datosPersona.tipo.filter((obj) => segundo.includes(obj.op)).length > 0
      ) {
        return "b";
      } else {
        return "d";
