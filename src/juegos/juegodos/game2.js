import { Grid, Button, Fade, Card, Typography } from "@mui/material";
...
const Game2 = () => {
  ...
  useEffect(() => {
    let imageList = [ropa, reloj, plato, mamadera, carritoSuper, billete];
  ...
  return (
    ...
    >
      <Grid item md={5} xs={12} className={classes.centerContent}>
        <CardBrain />
      </Grid>
      <Grid
        item
  ...
  return (
    <div
      ...
      style={{
        ...
        //margin: isMobile() ? -30 : -50,  compData.id.startsWith("center") === false
      }}
    >
      {iconCardData.map((conpData, i) => (
        <div
          ...
          style={{
            ...
            margin: isMobile() ? -4 : -25,
          }}
        >
          <img src={conpData.icon} height="100%" width="100%" />
        </div>
      ))}
    </div>
  );
...
const useStyles = makeStyles(() => ({
  ...
  cardResp: {
    ...
    borderBottom: `8px solid ${colores.mainStrong}`,
  },
  SpringTxtRep: {
