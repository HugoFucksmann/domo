...
const CardContent = ({ data, i }) => {
	const { openModal, handlerModalOpenGlosario } =
		useContext(AppContext);
					...
					{EmojiBtnData.map((emojiObj, i) => (
						...
						>
							<div
								...
								onClick={() =>
									handlerModalOpenGlosario(emojiObj.name, data)
								}
								style={{
									background: `url(${emojiObj.emoji}) no-repeat center center`,
								}}
							/>
						</Card>
					))}
